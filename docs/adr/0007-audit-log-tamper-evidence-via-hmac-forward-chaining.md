...
- Cross-file crypto binding (chaining file N+1 from file N's `audit.roll` hash, rather than each file independently from GENESIS) is deliberately NOT implemented. It is an accepted residual risk because the threat is bounded by key custody: an attacker holding `audit.key` can forge a self-consistent chain across files regardless, so cross-file binding would add complexity without raising the effective barrier. Each file remains independently verifiable from GENESIS.
- `verifyAuditChain` detects GAPS in the rotation file sequence (a retained middle file deleted by a keyless attacker, e.g. `.2` removed while `.1` and `.3` remain) via file-number contiguity. Previously the scan halted at the first missing file, silently ignoring any higher-numbered file and leaving the deletion undetected. The contiguity check closes that silent-deletion hole while each file remains independently verifiable from GENESIS; the gap detection is a structural check, not a crypto cross-file link.
